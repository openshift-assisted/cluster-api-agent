...
func initializeMirrorConfigs() *MirrorConfigs {
...
func GenerateImageRegistryConfigmap(imageRegistry *corev1.ConfigMap, namespace string) (*corev1.ConfigMap, error) {
	...
	mirrorConfigs := initializeMirrorConfigs()
	err := getImageRegistries(registryConf, mirrorConfigs)
	if err != nil {
		return nil, err
	}
...
func getImageRegistries(registry string, mirrorConfigs *MirrorConfigs) error {
	// Parse toml and add mirror registry to image digest mirror set
	tomlTree, err := toml.Load(registry)
	if err != nil {
		return errors.Wrap(err, "failed to load value of registries.conf into toml tree; incorrectly formatted toml")
	}
	registriesTree, ok := tomlTree.Get("registry").([]*toml.Tree)
	if !ok {
		return fmt.Errorf("failed to find registry key in toml tree")
	}
	...
	for _, registry := range registriesTree {
		...
		if !sourceExists {
			return fmt.Errorf("failed to parse image registry toml, missing registry location")
		}
		if !mirrorExists {
			return fmt.Errorf("failed to parse image registry toml, missing registry.mirror in registries.conf")
		}
		if err := parseMirrorRegistries(mirrorConfigs, mirrorTrees, source); err != nil {
			return errors.Wrap(err, fmt.Sprintf("failed to parse image mirror registry toml for registry %s", source))
		}
	}

	if len(mirrorConfigs.DigestMirrors) < 1 && len(mirrorConfigs.TagMirrors) < 1 {
		return fmt.Errorf("failed to find any image mirrors in registry.conf")
	}
	return nil
}
