...
def test_version_repository_find_all(versions_file):
    ...
    assert len(version.commits) == 7

    assert version.commits[0].repository == "https://github.com/kubernetes-sigs/cluster-api"
    assert version.commits[2].image_url.startswith("quay.io/edge-infrastructure/assisted-service")
